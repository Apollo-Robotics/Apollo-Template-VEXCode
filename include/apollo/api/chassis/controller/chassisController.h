...
class chassisController {
 public:
  ...
  /*
   *TODO: figure out better wording for set to actually move the drive. set
   *implies the user is "setting" a default. some other terminology would help
   *to clarify this notion.
   */

  virtual void setDriveSpeed(double targetSpeed) = 0;
  virtual void setTurnSpeed(double targetSpeed = 0);
  virtual void setSwingSpeed(double targetSpeed, std::string direction) = 0;
  ...
  virtual void setSwingTime(QTime targetTime, std::string direction,
  ...
  virtual void setSwingPID(QAngle targetAngle, std::string direction,
                           double targetSpeed) = 0;
